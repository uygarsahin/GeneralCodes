...
log_file = os.path.join('/home/jzwxfg/Documents/scripts/', CR_num+'_'+ datetime.now().strftime('%Y%m%d') + '.log')
...
def check_supported_idrac_version():
    ...
    if response.status_code == 401:
        logging.warning("WARNING:%s: status code 401 detected, check iDRAC username / password credentials",idrac_ip)
        #logging.warning("\n- WARNING, status code 401 detected, check iDRAC username / password credentials")
        sys.exit(0)
    elif response.status_code != 200:
        logging.warning("WARNING:%s: GET request failed to validate iDRAC creds, status code %s returned.", idrac_ip, response.status_code)
...
def get_current_power_state():
    ...
    if response.status_code != 200:
        logging.warning("WARNING:%s: GET request failed to get current server power state, status code %s returned.", idrac_ip, response.status_code)
        #logging.warning("\n- WARNING, GET request failed to get current server power state, status code %s returned." % response.status_code)
        logging.warning(data)
        sys.exit(0)
    logging.info("%s : current server power state: %s\n", idrac_ip, data['PowerState'])
...
def set_power_state():
    logging.info("%s :setting new server power state to: %s", idrac_ip, (args["set"]))
    ...
    if response.status_code == 204:
        ...
        logging.info("PASS:%s: status code %s returned, server power state successfully set to \"%s\"\n" % (idrac_ip,response.status_code, args["set"]))
    else:
        #logging.error("\n- FAIL, Command failed, status code %s returned\n" % response.status_code)
        logging.error("FAIL:%s: Command failed, status code %s returned\n",idrac_ip, response.status_code)
...
if __name__ == "__main__":
    ...
    if args["ip"] and args["ssl"] or args["u"] or args["p"] or args["x"]:
        ...
        logging.error("FAIL:%s: invalid argument values or not all required parameters passed in. See help text or argument --script-examples for more details.",idrac_ip)
    ...
    if args["get"]:
        ...
        set_power_state()
        logger.info('Sleep 10 sec. after that get the new status of %s',idrac_ip)
        time.sleep(10)
        get_current_power_state()
    else:
        logging.error("FAIL:%s: invalid argument values or not all required parameters passed in. See help text or argument --script-examples for more details.",idrac_ip)
